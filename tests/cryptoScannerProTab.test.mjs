...
import {
  runCryptoScannerProScan,
} from "../services/crypto/cryptoScannerProService.js";
import {
...
assert.ok(!cryptoTabHtml.includes("crypto-production-heatmap"), "Old crypto heatmap should not remain visible in Crypto tab");
assert.ok(!cryptoTabHtml.includes("crypto-production-tv"), "Old crypto chart block should not remain visible in Crypto tab");
...
for (const needle of [
  "crypto-scan-now",
  "crypto-pro-tv-widget",
  "crypto-body",
  "crypto-category-heat",
  "crypto-top-movers",
...
async function mockFetch(url) {
  ...
  assert.ok(value.includes("coins/markets"), `Unexpected scanner URL: ${value}`);
  return jsonResponse([
    {
      id: "stellar",
      symbol: "xlm",
