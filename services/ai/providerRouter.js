...
const LOCAL_REASONING_PATTERN = /\b(scanner|watchlist|rank|ranking|best\s+(plays?|setups?|stocks?|crypto)|strongest\s+(stock|crypto|setup)|what\s+should\s+i\s+watch|external\s+signals?|prosperio|signal\s+overlay|deep\s*dive|top\s+picks?|thesis|why\s+are\s+these\s+top\s+picks|compare\s+[A-Z]{1,5}\s+vs\s+[A-Z]{1,5}|setup|rsi|macd|vwap|support|resistance|momentum|strength|volume|risk\/reward|risk reward|heatmap|ai\s*heatmap|aiheatmap|selected\s+aiheatmap|supplied data|loaded data|internal data)\b/i;
