...
export function normalizeCoinGeckoMarket(coin = {}, timestamp = new Date().toISOString()) {
  ...
  return {
    ...
    high24h: safeNumber(coin.high_24h),
    low24h: safeNumber(coin.low_24h),
    image: coin.image || "",
...
async function fetchMarketsPage(page, options = {}) {
  ...
  const url = apiProxyBase
    ? `${apiProxyBase}/coingecko/markets?${directUrl.split("?")[1]}`
    : directUrl;
  ...
  }, options.timeoutMs || CRYPTO_SCANNER_DEFAULTS.timeoutMs);
}
...
function dedupeRows(rows = []) {
  ...
  for (const row of rows) {
    ...
    if (!existing || Number(row.rank || 999999) < Number(existing.rank || 999999)) {
...
function sessionStats(rows = [], allRows = []) {
  ...
  const avgChange = rows.length
    ? rows.reduce((sum, row) => sum + Number(row.change24h || 0), 0) / rows.length
    : null;
  return {
...
export async function runCryptoScannerProScan(options = {}) {
  ...
  for (const page of pages) {
    try {
      const pageData = await fetchMarketsPage(page, {
        ...options,
        coinGeckoApiKey,
      });
  ...
  const filtered = sortScannerRows(filterCryptoScannerRows(allRows, options.filters || {}));
  const rows = filtered.slice(0, options.limit || 100);
  const topMovers = buildCryptoTopMovers(rows);
  const alerts = buildCryptoSignalAlerts(rows, timestamp);
  const categoryHeat = computeCryptoCategoryHeat(rows);
