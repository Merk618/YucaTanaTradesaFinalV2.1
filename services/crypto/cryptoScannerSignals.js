...
export function classifyCryptoSignal(row = {}) {
  ...
  if (Number.isFinite(rsi) && rsi >= 75) {
    return { signal: "OVERBOUGHT", source: "RSI model", reason: "RSI is elevated from supplied technical data." };
  }
  if ((change24h >= 16 || change1h >= 5) && volume >= 100_000_000) {
    return { signal: "HYPER", source: "price-change model", reason: "Large short-term move with meaningful reported volume." };
  }
  if (change24h >= 10 || (change1h >= 3 && volume >= 50_000_000)) {
    return { signal: "SURGE", source: "price-change model", reason: "Strong recent upside pressure from provider snapshot." };
  }
  if (change24h >= 6 && change7d >= 0) {
    return { signal: "BREAKOUT", source: "price-change model", reason: "24h strength is aligned with a non-negative 7d backdrop." };
  }
  if (change24h >= 2.5 && change7d >= 1) {
    return { signal: "MOMENTUM", source: "price-change model", reason: "Short-term and weekly price change are both constructive." };
  }
  if (change24h > 0.75 && change7d <= -7) {
    return { signal: "REVERSAL", source: "price-change model", reason: "Positive 24h move is emerging after a weak 7d stretch." };
  }
  if (Math.abs(change24h) <= 1.25 && (marketCap >= 1_000_000_000 || row.rank <= 80)) {
    return { signal: "SUPPORT", source: "price-change model", reason: "Large-cap token is consolidating near flat daily change." };
  }
  return { signal: "NEUTRAL", source: "price-change model", reason: "No strong scanner signal was detected from supplied fields." };
}
...
export function parseSymbolFilter(value = "") {
  return String(value || "").split(/[\s,]+/).map(normalizeCryptoScannerSymbol).filter(Boolean);
}
...
export function computeCryptoCategoryHeat(rows = []) {
  return Object.entries(CRYPTO_CATEGORY_MAP).map(([category, symbols]) => {
    ...
    if (!matching.length) {
      return { category, value: null, label: "Unavailable", count: 0, dataQuality: "UNAVAILABLE" };
    }
    const avg = matching.reduce((sum, row) => sum + Number(row.change24h ?? row.changePct), 0) / matching.length;
    return { category, value: avg, label: `${avg >= 0 ? "+" : ""}${avg.toFixed(2)}%`, count: matching.length, dataQuality: "RECENT" };
  });
}
...
export function buildCryptoSignalAlerts(rows = [], timestamp = new Date().toISOString()) {
  ...
  for (const row of sorted) {
    ...
    if (["HYPER", "SURGE", "BREAKOUT", "MOMENTUM", "REVERSAL", "OVERBOUGHT"].includes(signal)) {
      alerts.push({ symbol: row.symbol, label: signal, detail: `${row.symbol} ${signal.toLowerCase()} from ${row.signalSource || "price-change model"}.`, timestamp });
    } else if (volume >= 1_000_000_000 && Math.abs(change) >= 2) {
      alerts.push({ symbol: row.symbol, label: "VOLUME SURGE", detail: `${row.symbol} reported heavy 24h volume with ${change >= 0 ? "+" : ""}${change.toFixed(2)}% move.`, timestamp });
    }
  }
  return alerts;
}
